		...
		<Card className="w-96">
			...
			<CardContent>
				...
				{otpActivation.type === 'otp_activation_failed' && (
					<div className="text-red-500 text-center">{dict.tenant.otpActivation.errorMessages[otpActivation.error]}</div>
				)}
				{otpActivation.type === 'otp_activated' && (
					<div className="flex flex-col items-center gap-4">
						<p className="text-lg text-center">Enter the following code where you initiated signing in:</p>
						<div className="text-4xl font-bold border p-2 rounded font-mono">{otpActivation.otp}</div>
					</div>
				)}
